...
class Robot: public SampleRobot
{
	...
	Encoder armEncoder,liftEncoder;//these two encoders omnomnom the available auton modes, currently 6 modes remain
...
public:
	...
	void waitUntil(bool &val)//might be useful for line sensor auton
	{
		while(!&val&&IsEnabled())
		{
			//more nothing
	...
	bool getIsLineBase()
	{
		return baseline - line.GetVoltage() > 1;
	}
	...
	void Autonomous()
	{
		...
		case 5:
			...
			/*myRobot.ArcadeDrive(-0.5,0,0);//drive forwards towards the tape
			while(getIsLineBase()==0){}//off tape
			while(getIsLineBase()==1){}//on tape
			myRobot.ArcadeDrive(0,0,0);//passed the tape, stop.*/
			while(FSE<4&&IsEnabled())//simple FSE?
			{
				switch(FSE)
				{
				case 0:
					...
					break;
				case 1:
					//std::cout<<"FSE 1"<<std::endl;
					std::cout<<"FSE 1 "<<line.GetVoltage()<<std::endl;
					if(getIsLineBase())
					{
						FSE=2;
						wait(0.1);
					...
					break;
				case 2:
					//std::cout<<"FSE 2"<<std::endl;
					std::cout<<"FSE 2 "<<line.GetVoltage()<<std::endl;
					if(!getIsLineBase())
						FSE=3;
					break;
				case 3:
					std::cout<<"FSE 3"<<std::endl;
					std::cout<<line.GetVoltage()<<std::endl;
					//myRobot.ArcadeDrive(0,0,0);
					FSE=4;
					break;
				}
		...
		std::cout<<"auton complete"<<std::endl;
	}
	...
	void OperatorControl()
	{
		...
		while (IsOperatorControl() && IsEnabled())
		{
			...
			else
			{
				...
				//open time: 2.5 seconds
			}
			liftcmd = 0;
			armcmd = 0;
			//std::cout<<getIsLineBase();
			//std::cout<<baseline - line.GetVoltage();
			//std::cout<<std::endl;
			current = line.GetVoltage();
			tripped = false;
			if(current<min)
			{
				min = current;
