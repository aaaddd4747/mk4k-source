...
class Robot: public SampleRobot
{
    ...
    PIDController mysecondpid,mythirdpid;
    AnalogPotentiometer potent;
    Encoder dave;//dave has a res of 90 ticks per rotation
    Talon clawmotor,liftenmotor;
    //VictorSP clawmotor;
...
public:
	Robot() :
			...
			liftenmotor(5),
			potent(0),
			dave(0,1,false,Encoder::k4X),
			mysecondpid(0.5,0,0,&potent,&clawmotor),//p,i,d,in,out
			mythirdpid(0.5,0,0,&dave,&liftenmotor)//recieves rotation in degrees from dave
	{
		mysecondpid.SetSetpoint(1);
		mysecondpid.SetOutputRange(-1,1);
	...
	void RobotInit()
	{
		...
		dave.Reset();
		dave.SetDistancePerPulse(4);//4deg/tick
	}
	/**
	...
	{//placeholder test thingy
		...
		{
			robotDrive.MecanumDrive_Cartesian(0,.25,0);
			Wait(0.01);
		}
		robotDrive.MecanumDrive_Cartesian(0,0,0);
	}
	void OperatorControl()
	{
		...
		while (IsOperatorControl() && IsEnabled())
		{
			...
			//cout<<dave.GetDistance();
			cout<<endl;//didnt want me to do that on prev line for some reason..
			//kil pid if on targ
			mysecondpid.OnTarget()?mysecondpid.Disable():mysecondpid.Enable();
			mythirdpid.OnTarget()?mythirdpid.Disable():mythirdpid.Enable();
			/*if(mysecondpid.OnTarget())
				mysecondpid.Disable();
			else
				mysecondpid.Enable();

			if(mythirdpid.OnTarget())
				mythirdpid.Disable();
			else
				mythirdpid.Enable();
			*/
			...
			mZ += mY>0 ? -0.01 : -0.01;//ternary ftw
			mZ += mX>0 ? -0.005 : 0.005;//translate control fix maybe?
			//brownout protection
			...
			//std::cout<<robotDrive.m_frontLeftMotor->Get()<<"    "<<robotDrive.m_frontRightMotor->Get()<<"     "<<robotDrive.m_rearLeftMotor->Get()<<"      "<<robotDrive.m_rearRightMotor->Get()<<endl;
			if(stick.GetRawButton(8))
			{
				//j->SetSpeed(0.5);
				cout<<"gr8 b8 m8 i r8 8/8"<<endl;
				mysecondpid.Enable();
